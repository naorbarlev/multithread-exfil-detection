...
export {
	...
	redef enum Notice::Type += { File_Transfer, };
	...
	global add_conn_to_threads: function(c: connection, s: Settings,
	    new_thread: bool);
}
...
type sessions: table[string] of connection_stats &create_expire=1 day;

## This is the data strucutre that holds our aggregated threads
type threading_stats: table[string] of thread_aggregation_stats
    &create_expire=1 hr;
...
function alert(c: connection, byte_cnt: count, n: Notice::Type, notice: bool)
	{
	local rec = Exfil::Info($ts=network_time(), $uid=c$uid, $id=c$id,
	    $proto=( get_port_transport_proto(c$id$orig_p) ),
	    $service=c$service, $orig_bytes=byte_cnt, $note=n);
	if ( c?$ssl )
		{
		if ( c$ssl?$subject )
			{
	...
	if ( notice )
		...
		NOTICE([ $note=n, $id=c$id, $msg=fmt("Large file upload detected from %s to %s. This file is approximately %s bytes.",
		    c$id$orig_h, c$id$resp_h, byte_cnt), $sub=c$ssl$subject ]);
		}
	}
...
event Exfil::regular_check(c: connection, s: Settings)
	...
	if ( ! connection_exists(c$id) )
		...
		if ( c$uid in tracked_sessions )
			...
			if ( tracked_sessions[c$uid]$reported )
				{
				byte_cnt = ( tracked_sessions[c$uid]$last_byte_cnt -
				    tracked_sessions[c$uid]$orig_byte_cnt );
	...
	if ( c$uid !in tracked_sessions )
		...
		tracked_sessions[c$uid] = [ $last_byte_cnt=c$orig$size, $src_ip=c$id$orig_h,
		    $src_p=c$id$orig_p, $dst_ip=c$id$resp_h, $dst_p=c$id$resp_p ];
		...
		schedule s$thread_checkup_interval { Exfil::thread_check(c, s,
		    thread_collection[conn_key]$total_byte_cnt) };
		return;
		}
	...
	local byte_rate: double = ( ( c$orig$size - session$last_byte_cnt ) + 0.0 /
	    interval_to_double(s$checkup_interval) );
	...
	if ( debug )
		{
		local debug_rec: Exfil::DebugInfo = [ $ts=network_time(), $id=c$id,
		    $uid=c$uid, $byte_rate=byte_rate,
	...
	schedule s$thread_checkup_interval { Exfil::thread_check(c, s,
	    thread_collection[new_key]$total_byte_cnt) };
	schedule s$checkup_interval { Exfil::regular_check(c, s) };
