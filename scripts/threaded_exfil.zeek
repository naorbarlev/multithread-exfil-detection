...
event Exfil::thread_check(c: connection, s: Settings, b: count)
	...
	if ( c_key in thread_collection )
		...
		# it is safe to assume all threads have been completed.  If the byte count is high enough, raise a notice.
			...
			{
			if ( ( thread_collection[c_key]$thread_cnt >= s$unique_thread_thresh )
			    && ( thread_collection[c_key]$total_byte_cnt > s$file_thresh ) )
				{
				NOTICE([ $id=c$id, $src=c$id$orig_h, $dst=c$id$resp_h, $p=c$id$resp_p,
				    $note=Exfil::File_Transfer, $msg=fmt("A total of %d bytes have gone from %s to %s:%s over %d unique streams", (
				    thread_collection[c_key]$total_byte_cnt ), (
				    c$id$orig_h ), ( c$id$resp_h ), (
				    c$id$resp_p ), (
				    thread_collection[c_key]$thread_cnt )),
			...
			{
			thread_collection[c_key]$timed_byte_cnt = (
			    thread_collection[c_key]$total_byte_cnt );
			return;
			}
		}
...
function add_conn_to_threads(c: connection, s: Settings, new_thread: bool)
	...
	for ( x, y in tracked_sessions )
		...
		if ( add_entry == T )
			{
			thread_collection[new_key] = [ $thread_cnt=1,
			    $total_byte_cnt=y$last_byte_cnt, $timed_byte_cnt=0 ];
			y$orig_byte_cnt = y$last_byte_cnt;
			new_thread = F;
			}
		}
